/**
 * Radios covalentes (Å) según Cordero et al. 2008.
 * Usados para detección de enlaces (distancia < suma * 1.15).
 */
...
export function covalentRadius(symbol: string): number {
  return COVALENT_RADII[symbol] ?? 1.4;
}

/**
 * Radios visuales para renderizado (Å). Más chicos que los covalentes para que
 * el cristal se vea con átomos esféricos pero con espacio entre ellos.
 */
export function renderRadius(symbol: string): number {
  return covalentRadius(symbol) * 0.55;
}
